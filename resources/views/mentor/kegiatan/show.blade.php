        ...
        <main class="flex-1 p-6 bg-gray-100">
            <div class="bg-white p-6 rounded shadow space-y-4">
                    ...
                    @if(auth()->user()->role == 'admin' || auth()->user()->role == 'mentor')
                        ...
                        @if($statusTarget == 'Tercapai')
                            ...
                            @if(!$kegiatan->evaluasi)
                                <a href="{{ route('mentor.evaluasi.create', $kegiatan->id) }}"
                                class="bg-green-600 text-white px-4 py-2 rounded">
                                    Beri Evaluasi
                                </a>
                            @else
                                <a href="{{ route('mentor.evaluasi.show', $kegiatan->evaluasi->id) }}"
                ...
                <div class="flex gap-2 mt-4">
                    <a href="{{ route('mentor.kegiatan.edit', $kegiatan) }}"
                       class="px-3 py-2 bg-yellow-500 text-white rounded">
                        Edit
                    </a>

                    <form action="{{ route('mentor.kegiatan.destroy', $kegiatan) }}"
                          method="POST">
                        @csrf
                        @method('DELETE')
                    ...
                    <a href="{{ route('mentor.kegiatan.cetak', $kegiatan) }}"
